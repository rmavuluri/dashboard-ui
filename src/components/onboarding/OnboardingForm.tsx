...
import { saveOnboardingData, getOnboardingById } from './../../services/onboardingService';
...
import { EnvironmentSelect } from '../form/EnvironmentSelect';
...
const OnboardingForm = () => {
  ...
  const [formData, setFormData] = useState<Partial<OnboardingData>>({
    ...
    notificationEmail: '',
    contactEmails: '',
    environments: ENVIRONMENTS.map(type => ({ type, enabled: false })),
  });
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.environments?.some(env => env.enabled)) {
  ...
  const handleEnvironmentsChange = (environments: Environment[]) => {
  ...
  return (
    ...
    >
      ...
      <FormSection title="Environment Selection">
        <div className="col-span-2">
          ...
          />
          {errors.environments && (
            <p className="mt-1 text-sm text-red-600">{errors.environments}</p>
          )}
        </div>
      </FormSection>
